...
def calculate_model_metrics(datasets, noise_ranges):
    """Calculate R-squared and Adjusted R-squared for each dataset"""
    r_squared_values = []
...
def generate_linear_transform_variables(independent_variables, betas):
    ...
    return max_correlations
    """Generate 20 new variables as linear transformations of existing variables"""
    new_variables = []
    formulas = []
    n_vars = independent_variables.shape[1]
    generated_combinations = set()
    ...
    while len(new_variables) < 20:
        ...
        if operation == 'multiply':
            ...
            if combination not in generated_combinations:
                new_var = independent_variables[:, idx] * factor
                new_variables.append(new_var)
                formulas.append(f'{factor:.4f} * x{idx+1}')
                generated_combinations.add(combination)
            ...
            if combination not in generated_combinations:
                new_var = independent_variables[:, idx] + offset
                new_variables.append(new_var)
                sign = '+' if offset >= 0 else '-'
                formulas.append(f'x{idx+1} {sign} {abs(offset):.4f}')
                generated_combinations.add(combination)
...
# Main execution
def main():
    print("Linear Regression Analysis with Variable Transformations")
    print("=" * 55)
